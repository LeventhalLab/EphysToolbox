function [varargout] = combineNex( varargin )
%
% usage: [varargout] = combineNex( varargin )
%
...
%
% varargins:
%   'fs' - sampling rate for original file; default 31250
%   'savename' - name to give to the new .nex file. If left empty, no file
%       is written
%   'nexnames' - cell array containing the names of the .nex files to
%       combine
%
% OUTPUTS:
%
% varargouts:
%   1st arg - the combined nex data structure
...
for iarg = 1 : 2 : nargin
    switch lower(varargin{iarg});
        case 'fs',
            nSampleRate = varargin{iarg + 1};
        case 'savename',
            saveName = varargin{iarg + 1};
        case 'nexnames',
            nexfn = varargin{iarg + 1};
            if ~iscell(nexfn)
                nexfn = { nexfn };
            end
    end
            ...
            case 0, % neuron
                ...
                if ~isempty(nexData.neurons)
                    ...
                    for iNeuron = 1 : numNeurons
                        if strcmpi(nexData.neurons{iNeuron}.name, curName)
                            exitFlag = 1;
                            break;
                        end
                    end
                ...
                nexData.neurons{numNeurons, 1}.name = curName;
...
if ~isempty(saveName)
    result = writeNexFile(nexData, saveName);
    if result
        disp([saveName ' successfully saved.']);
    else
        disp(['error saving ' saveName]);
    end
end
varargout(1) = {nexData};
