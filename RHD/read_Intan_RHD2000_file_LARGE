function read_Intan_RHD2000_file_LARGE(varargin)
...
if (data_present)
    ...
    amplifier_data_1 = zeros(num_amplifier_channels/2, num_amplifier_samples);
    amplifier_data_2 = zeros(num_amplifier_channels/2, num_amplifier_samples);
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
            ...
            amplifier_data_1(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_amplifier_channels/2], 'uint16')';
            amplifier_data_2(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_amplifier_channels/2], 'uint16')';
...
if (data_present)
    ...
    amplifier_data_1 = 0.195 * (amplifier_data_1 - 32768); % units = microvolts
    amplifier_data_2 = 0.195 * (amplifier_data_2 - 32768); % units = microvolts
    ...
    if (notch_filter_frequency > 0)
        ...
        for i=1:num_amplifier_channels
            amplifier_data(i,:) = ...
                notch_filter(amplifier_data_1(i,:), sample_rate, notch_filter_frequency, 10);
                notch_filter(amplifier_data_2(i,:), sample_rate, notch_filter_frequency, 10);
...
if (num_amplifier_channels > 0)
    ...
    if (data_present)
        move_to_base_workspace(amplifier_data_1);
        move_to_base_workspace(amplifier_data_2);
